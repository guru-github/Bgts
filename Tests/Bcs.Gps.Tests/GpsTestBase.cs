...
namespace Bcs.Gps.Tests
{
    public abstract class GpsTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<GpsDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<GpsDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GpsDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GpsDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GpsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GpsDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<GpsDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GpsDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GpsDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GpsDbContext, Task<T>> func)
        {
            T result;
