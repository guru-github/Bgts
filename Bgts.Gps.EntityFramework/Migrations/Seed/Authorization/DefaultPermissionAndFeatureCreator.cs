...
using Bgts.Gps.Authorization;
using Bgts.Gps.Features;

namespace Bgts.Gps.Migrations.Seed.Authorization
{
    public class DefaultPermissionAndFeatureCreator
    {
        private readonly AuthDbContext _context;

        public DefaultPermissionAndFeatureCreator(AuthDbContext context)
        {
            _context = context;
        }
        ...
        public void Create()
        {
            CreateHostPermissionsAndFeatures();
            CreateDefaultTenantPermissionsAndFeatures();
        }

        private void CreateHostPermissionsAndFeatures()
        {
            #region check database if it has user and role for host
            //Admin role for host
            ...
            var permissions = PermissionFinder
                    .GetAllPermissions(new VhasAuthorizationProvider())
            ...
            var features = FeatureFinder
                .GetAllFeatures(new VhasFeatureProvider())
                .Where(x => x.Scope.HasFlag(FeatureScopes.Edition))
                .ToList();

            foreach (var feature in features)
            {
                if (!string.Equals(feature.DefaultValue, "true", StringComparison.InvariantCultureIgnoreCase))
                {
        ...
        private void CreateDefaultTenantPermissionsAndFeatures()
        {
            #region check database if it has user and role for tenant
            //Default tenant
            ...
            var features = FeatureFinder
                .GetAllFeatures(new VhasFeatureProvider())
                .Where(x => x.Scope.HasFlag(FeatureScopes.Tenant))
                .ToList();
